...
def main():
    logger.info("Iniciando script de upload de dataset.")

    # Agora espera 3 ou 4 argumentos (script, bucket, arquivo, [prefixo])
    if len(sys.argv) < 3:
        logger.error("Uso correto: python upload_dataset.py <bucket_name> <caminho_arquivo_local> [pasta_destino_no_bucket]")
        print("Uso correto: python upload_dataset.py <bucket_name> <caminho_arquivo_local> [pasta_destino_no_bucket]")
        sys.exit(1)

    bucket_name = sys.argv[1]
    file_path = sys.argv[2]
    # Captura o prefixo opcional
    ...
    try:
        client = MinioClient()
        # **AQUI ESTÁ A MUDANÇA ESSENCIAL: PASSE O PREFIXO**
        ...
        if success:
            ...
            print(f"[✔] Arquivo '{file_path}' enviado como '{final_object_name}' para o bucket '{bucket_name}' com sucesso.")
        else:
            logger.error(f"[✖] Falha no envio do arquivo '{file_path}'. Verifique os logs para mais detalhes.")
            print(f"[✖] Falha no envio do arquivo '{file_path}'. Verifique os logs para mais detalhes.")
    except Exception as e:
        logger.critical(f"Erro inesperado no script de upload: {e}", exc_info=True)
        print(f"[✖] Ocorreu um erro inesperado: {e}")
    finally:
        logger.info("Script de upload de dataset finalizado.")
