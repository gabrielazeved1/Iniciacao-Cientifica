...
def main():
    ...
    try:
        ...
        if success:
            ...
            print(f"[✔] Diretório '{local_directory}' enviado para o bucket '{bucket_name}' com sucesso.")
        else:
            logger.error(f"[✖] Falha no envio do diretório '{local_directory}'. Verifique os logs para mais detalhes.")
            print(f"[✖] Falha no envio do diretório '{local_directory}'. Verifique os logs para mais detalhes.")
